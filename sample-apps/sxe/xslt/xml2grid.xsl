	...
	<xsl:template name="buttonTD">
		...
		<td class="expand-toggle-button {$classSuffix}-button">
			<xsl:if test="$classSuffix = 'element' and $hasChildNodes = '1'">
				<xsl:attribute name="onmouseover">doButtonMouseOver(this)</xsl:attribute>
				<xsl:attribute name="onmouseout">doButtonMouseOut(this)</xsl:attribute>
				<xsl:attribute name="onclick">doHideOrShowChildNodes(this)</xsl:attribute>
			</xsl:if>
